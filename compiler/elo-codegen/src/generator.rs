...
// Copyright(c) 2026 Marcio Dantas, Igor Ferreira
#include <prelude.h>\n";

const MAIN: &'static str = "void __elo_main(GlobalContext* ctx)";
...
fn mangle_array_type(no: usize) -> String {
    return format!("_array{no}_{}", fnv_hash(&no.to_string()));
}
...
impl Generator {
    ...
    fn get_array_type(&mut self, typ: &cir::Typing, amount: usize) -> String {
        let mut array_type_index = None;
        for (i, (this_typ, this_amount)) in self.arrays.iter().enumerate() {
            if (typ, &amount) == (&this_typ, this_amount) {
                array_type_index = Some(i);
            }
        }
        if let Some(i) = array_type_index {
            return mangle_array_type(i);
        } else {
            let i = self.arrays.len();
            self.arrays.push((typ.clone(), amount));
            let mut body = String::new();
            body.push_str(&format!("{} items[{amount}];\n", self.choose_type(typ)));
            let struct_name = mangle_array_type(i);
            self.head.push_str(&c::struct_stmt(&struct_name, &body));
            return struct_name;
        }
    }
    ...
    pub fn choose_type(&mut self, t: &cir::Typing) -> String {
        return match t {
            ...
            cir::Typing::Array { typ, amount } => format!("struct {}", self.get_array_type(typ, *amount)),
    ...
    pub fn generate_expression(&mut self, expr: &cir::Expression) -> String {
        match &expr.data {
            ...
            cir::ExpressionData::ArrayLiteral { exprs, typ } => {
                ...
                let array_t = self.get_array_type(typ, amount);
                let exprs = exprs.iter().map(|e| self.generate_expression(e));
                let items = c::list(&exprs.collect::<Vec<String>>());
                return c::struct_expr(&array_t, &[("items".to_string(), c::braced_expr(&items))]);
            }
            ...
            cir::ExpressionData::ArraySubscript { origin, index } => {
                let origin = c::member_expr(&self.generate_expression(origin), "items");
    ...
    pub fn generate_named_fn_args(&mut self, args: &Vec<(String, cir::Typing)>, extrn: bool) -> String {
        let mut result = vec![];
        if !extrn {
            result.push(c::field("GlobalContext*", "ctx"));
        }
    ...
    pub fn generate_fn_args(&mut self, args: &Vec<cir::Typing>, extrn: bool) -> String {
        let mut result = vec![];
        if !extrn {
            result.push(String::from("GlobalContext*"));
        }
        for typ in args {
            let t = self.choose_type(typ);
            result.push(t);
